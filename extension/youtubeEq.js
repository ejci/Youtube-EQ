...
app.init = function(videoId, videoTag) {
    ...
    $('.html5-volume-control').html('<button> </button>');
    ...
    $('#yt-eq-menu-info').click(function() {
        window.open('http://lab.ejci.net/YoutubeEQ', '_blank');
    });
    ...
    node.onaudioprocess = function() {
        ...
        function drawSpectrum(array) {
            ...
            for (var i = 0; i < (array.length); i = i + size) {
                ...
                value = (value / 256) * 100 - offset;
                ctx.fillRect(i, 100 - value, (size > 1) ? (size - 1) : 1, 100);

            }
        };
        function drawLineSpectrum(array) {
            var offset = 0;
            function getValue(a) {
                var value = Math.floor(a);
                value = (value / 256) * 100 - offset;
                return value;
            }
            ...
            ctx.lineWidth = 1;
            ctx.strokeStyle = '#eee';
            var val = getValue(array[0]);
            ctx.moveTo(0, 100 - val);
            for (var i = 1; i < (array.length - 2); i++) {
                var xc = (i + (i + 1)) / 2;
                var yc = (getValue(array[i]) + getValue(array[i + 1])) / 2;
                ctx.quadraticCurveTo(i, 100 - getValue(array[i]), xc, 100 - yc);
                //ctx.lineTo(i, 100-getValue(array[i]))
            }
            ctx.quadraticCurveTo(i, 100 - getValue(array[i]), i + 1, 100 - getValue(array[i + 1]));
            //ctx.lineTo(array.length - 1, 100-getValue(array[array.length - 1]));
            ctx.stroke();
        };
